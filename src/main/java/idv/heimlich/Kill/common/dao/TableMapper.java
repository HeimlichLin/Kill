...
import idv.heimlich.Kill.common.utils.ObjectsUtils;
...
//	WAREHSE(WarehseDo.class, WarehseDAO.class),
	...
	private <PO, T extends GeneralDAOImpl<PO>> TableMapper(final Class<PO> doClas, final Class<T> daoClas) {
		this.doClas = doClas;
	...
	public static <PO> GeneralDAOImpl<PO> lookupDAO(final PO object) {
		final Class<?> daoClass = lookupDAOByDo(object);
		return (GeneralDAOImpl<PO>) ObjectsUtils.newInstance(daoClass);
	}
